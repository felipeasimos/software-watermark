...
#define show_bits(bits, len)                                                   \
  fprintf(stderr, "%s:%d:" #bits ": ", __FILE__, __LINE__);                    \
...
void get_key_from_k(unsigned long **k, unsigned long symsize,
  ...
  **k = invert_unsigned_long(**k);
  ...
  // fprintf(stderr, "zeros_in_symbol: %ld, left_zeros: %lu, total_n_bits: %lu,
  // data_bits: %lu, k: %lu\n", zeros_in_symbol, left_zeros, total_n_bits,
  // data_bits, **k);
...
int rs_test(void) {
  {
    ...
    //
    // rs_encode(data, sizeof(data)/sizeof(data[0]), parity, num_parity_symbols,
    // 3);
    //
    ...
    // data[1] = 5;
    //
    // rs_decode(data, sizeof(data)/sizeof(data[0]), parity, num_parity_symbols,
    // 3);
    //
...
int append_remove_rs_code_test() {
  ...
  uint8_t *data_with_rs =
      append_rs_code(data, &n_bits, num_parity_symbols, symsize);

  ctdd_assert(n_bits == 15);
  ctdd_assert(data_with_rs[0] == 183);
  ctdd_assert(data_with_rs[1] == 246);

  uint8_t *data_without_rs = remove_rs_code(data_with_rs, num_data_symbols,
                                            num_parity_symbols, symsize);
  free(data_with_rs);
  ...
  for (unsigned long num_data_symbols = 1;
       num_data_symbols < num_data_symbols_top; num_data_symbols++) {
    ...
    for (unsigned long num_parity_symbols = 1;
         num_parity_symbols <= num_parity_symbols_top; num_parity_symbols++) {
      ...
      for (unsigned long k = lower_bound; k < upper_bound; k++) {
        ...
        *key = k;
        ...
        data_with_rs =
            append_rs_code(key, &n_bits, num_parity_symbols, symsize);
        show_bits((void *)key, num_data_symbols * symsize);

        ctdd_assert(data_with_rs);
        ctdd_assert(n_bits ==
                    (num_data_symbols + num_parity_symbols) * symsize);
        data_without_rs = remove_rs_code(data_with_rs, num_data_symbols,
                                         num_parity_symbols, symsize);
        ctdd_assert(data_without_rs);
        ...
        for (unsigned long i = 0; i < n_bits; i++) {
          ctdd_assert(get_bit((void *)key, i) ==
                      get_bit((void *)data_without_rs, i));
        }
        free(data_without_rs);
        free(key);
      }
...
int watermark2017_rs_decode_improved_test() {
  ...
  for (uint8_t k = 1; k < 255; k++) {
    ...
    uint8_t *result =
        watermark_rs_decode_improved8(graph, &k, &size, num_parity_symbols);
    ctdd_assert(size == 1);
    ctdd_assert(*result == k);
    free(result);
    graph_free(graph);
...
int watermark2017_decode_analysis_test() {
  ...
  for (unsigned long k = 1; k < 10e13; k = (k << 1) - (k >> 1)) {
    ...
    uint8_t *result_seq = get_sequence_from_bit_arr(result, size, &size);
    ctdd_assert(
        binary_sequence_equal((uint8_t *)&k, result_seq, sizeof(k), size));
    free(result);
    free(result_seq);
    graph_free(graph);
  }
...
// unsigned short get_key_from_k(unsigned short k, unsigned long symsize,
// unsigned long num_data_symbols) {
//   unsigned long key = 0;
//   for(unsigned long i = 0; i < sizeof(k); i++) {
//     invert_binary_sequence(&((uint8_t*)&k)[i], 1);
...
int watermark2017_rs_3_bit_test() {
  ...
  uint8_t symsize = 3;
  {
    ...
    unsigned long num_parity_symbols = 2;
    unsigned long num_data_symbols = 3;
    GRAPH *graph = watermark_rs_encode(&data, num_data_symbols,
                                       num_parity_symbols, symsize);
    ctdd_assert(graph);
    uint8_t *result = watermark_rs_decode_improved(
        graph, &data, &num_data_symbols, num_parity_symbols, symsize);
    ctdd_assert(result);
  ...
  for (unsigned long num_data_symbols = 1;
    ...
    for (unsigned long k = lower_bound; k < upper_bound; k++) {
      ...
      *key = k;
      get_key_from_k((unsigned long **)&key, symsize, num_data_symbols);
      GRAPH *graph = watermark_rs_encode(key, num_data_symbols,
                                         num_parity_symbols, symsize);
      ...
      if (num_parity_symbols >= 2) {
        CONNECTION *conn1 =
            conn_next_non_hamiltonian_edge(graph->nodes[0]->out);
        if (conn1)
          graph_oriented_disconnect(conn1->parent, conn1->node);
      }
      ...
      uint8_t *result = watermark_rs_decode_improved(
          graph, key, &num_result_bytes, num_parity_symbols, symsize);
